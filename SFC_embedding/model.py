...
class DRL4TSP(nn.Module):
    ...
    def __init__(self, static_size, dynamic_size, hidden_size,
        ...
        self.decoder = Encoder(static_size+3, hidden_size)
    ...
    def forward(self,chain_b, static, dynamic, decoder_input=None, last_hh=None):
        """
        Parameters
        ----------
        ...
        max_steps = len(chain_b[-1])
        chain_b = chain_b.permute(1, 0, 2)
        if decoder_input is None:
            decoder_input = self.x0.expand(batch_size, -1, -1)
            ch = chain_b[0].unsqueeze(-1)
            decoder_input = torch.cat((decoder_input,ch),dim=1)

        # Always use a mask - if no function is provided, we don't update it
        mask= torch.zeros(batch_size, sequence_size, device=device)
        mask_re = torch.full((batch_size, sequence_size, 8), 115)
        ...
        for _ in range(max_steps):
            ...
            #mask_tem1=torch.where(mask_re >=chain_b[_][2].item(), 1, 0)
            mask_tem2 = torch.zeros(batch_size, sequence_size, device=device)
            for i in range(batch_size):
                for j in range(24):
                    for k in range(8):
                        #print(static[i][k + 24][j],chain_b[i][_][0],mask_re[i][j][k],chain_b[i][_][2])
                        if (static[i][k + 24][j] == chain_b[_][i][0])&(mask_re[i][j][k]>=chain_b[_][i][2]):
                            mask_tem2[i][j]=1
                            break

            probs = F.softmax(probs + mask_tem2.log(), dim=1)
            ...
            if self.training:
                ...
                ptr = m.sample()
                while not torch.gather(mask_tem2, 1, ptr.data.unsqueeze(1)).byte().all():
                    ptr = m.sample()
                logp = m.log_prob(ptr)
            else:
            ...
            for i in range(batch_size):
                    for k in range(8):
                        if (static[i][k + 24][ptr[i]] == chain_b[_][i][0])&(mask_re[i][ptr[i]][k]>=chain_b[_][i][2]):
                            mask_re[i][ptr[i]][k]-=chain_b[_][i][2]
                            break
            ...
            if _ !=max_steps-1:
                ch = chain_b[_+1].unsqueeze(-1)
                decoder_input = torch.cat((decoder_input, ch), dim=1).detach()
